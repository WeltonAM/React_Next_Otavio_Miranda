...
describe('<Header />', () => {
  it('should render an image, a heading and text', () => {
    ...
    expect(screen.getByRole('img', { name: /Otávio Miranda/i })).toHaveAttribute('src', props.logo);
  ...
  it('should render image only', () => {
    ...
    expect(screen.queryByRole('heading', { name: 'Otávio Miranda' })).not.toBeInTheDocument();
    expect(screen.getByRole('img', { name: /Otávio Miranda/i })).toHaveAttribute('src', props.logo);
